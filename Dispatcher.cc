...
//    if (incomingTask->getHopCount() >= maximumHop
//            || thisServerInfo->getTotalMemoryConsumed() + incomingTask->getTaskSize()
//                <= thisServerInfo->getServerCapacity() * memoryThreshold) {
//        incomingTask->setRunningServer(serverId);
//        intVector& hopPathForUpdate = incomingTask->getHopPathForUpdate();
//        hopPathForUpdate.push_back(serverId);
...
//
//    // if the current server can accommodate the task, then send to the processor
//    double totalDelay = (totalRequiredCycle + incomingTask->getRequiredCycle()) / serverFrequency;
//    if (omnetpp::simTime() + totalDelay <= incomingTask->getCreationTime() + incomingTask->getDeadline()
...
//        double lhsPredictedDelay = (incomingTask->getRequiredCycle() + lhs->getTotalRequiredCycle()) / lhs->getServerFrequency();
//        double rhsPredictedDelay = (incomingTask->getRequiredCycle() + rhs->getTotalRequiredCycle()) / rhs->getServerFrequency();
//        return lhsPredictedDelay < rhsPredictedDelay;
//    });
//
//    for (const Info* serverInfo: infoVec) {
//        if (serverInfo->getTotalMemoryConsumed() + incomingTask->getTaskSize() <= serverInfo->getServerCapacity()) {
//            incomingTask->setDestinationServer(serverInfo->getServerIdx());
//            send(incomingTask, "offloadOut");
//            return;
//        }
//    }
